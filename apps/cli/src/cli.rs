    ...
    match command {
        SshCommands::List => {
            ...
            for conn in connections {
                println!("  [{}] {} - {}@{}:{}", conn.id, conn.name, conn.username, conn.host, conn.port);
            }
        }
    ...
    match command {
        DockerCommands::List { host_id } => {
            ...
            for container in containers {
                println!("  [{}] {} - {} ({})", container.id, container.name, container.image, container.state);
            }
        }
        DockerCommands::Start { host_id, container_id } => {
            docker_manager.start_container(&host_id, &container_id).await?;
            println!("Container {} started", container_id);
        }
        DockerCommands::Stop { host_id, container_id } => {
            docker_manager.stop_container(&host_id, &container_id).await?;
    ...
    match command {
        CoolifyCommands::List { instance_id } => {
            ...
            for deployment in deployments {
                println!("  [{}] {} - {}", deployment.id, deployment.name, deployment.status);
            }
        }
        CoolifyCommands::Deploy { instance_id, project_id } => {
            coolify_manager.deploy_project(&instance_id, &project_id).await?;
...
async fn handle_git_command(command: GitCommands) -> anyhow::Result<()> {
    ...
    match command {
        ...
        GitCommands::Create { repo_id, tag, message } => {
